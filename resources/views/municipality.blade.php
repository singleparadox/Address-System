...
@section('content')
    ...
    <ul class="collection with-header">
        <li class="collection-header"><h4>Municipalities</h4></li>
        @foreach ($munis as $item)
            <li class="collection-item" municipalityId={{$item[0]}}>
                <div class="inline_block_content">
                        <a class="text_size" href='/{{$region_id}}/{{$province_id}}/{{$item[0]}}'> {{$item[1]}} </a>
                </div>
                <div class="inline_block_e_d">
                        <a class="waves-effect waves-light btn btn-small" href='/{{$region_id}}/{{$province_id}}/{{$item[0]}}/edit_municipality/{{$item[1]}}'><i class="material-icons">edit</i></a>
    ...
    <a href="/{{$region_id}}/{{$province_id}}/create_municipality" class="btn-floating btn-large waves-effect waves-light blue"><i class="material-icons">add</i></a>
    ...
    <script>
        ...
        $('button').click(function (e) {
            ...
            $.ajax({
                type: "post",
                url: "/delete_municipality",
                data: {'value' : clicked_button_value},
                dataType: "text",
                success: function (response) {
                    if (response == 'success') {
                        $('li[municipalityId='+clicked_button_value+']').remove();
                        console.log('success');
